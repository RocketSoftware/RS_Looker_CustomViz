 ...
 *   3. Select "Rocket — World Map (Transparent)" from the visualization picker.
 *
 * Expects:
...
(function () {
  ...
  const CSS = `
    ...
    .rwm-tt-label  { font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 1.1px; color: #7878A8; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    ...
    @media (prefers-color-scheme: light) {
      .rwm-title,
      .rwm-legend-title { color: #1A1A3A; }
      .rwm-subtitle,
      .rwm-legend-label,
      .rwm-nodata-label { color: #6060A0; }
      .rwm-legend-title { fill: #1A1A3A; }
      .rwm-legend-label, .rwm-nodata-label { fill: #6060A0; }
    }
  ...
  looker.plugins.visualizations.add({
    ...
    create(el, config) {
      // Inject styles once
      if (!document.getElementById("rwm-tr-styles")) {
